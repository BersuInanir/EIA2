...
namespace L4_Canvas {
    ...
    //Interface für den Skifahrer anlegen
    interface SkiInfo {
        x: number;
        y: number;
        dx: number;
    ...
    //Interface für die Gondel anlegen
    interface GondelInfo {
        x: number;
        y: number;
        dx: number;
        color: string;
    ...
    function init(): void {
        ...
        for (let i: number = 0; i < 2; i++) {
            gondel[i] = {                         // Werte für das Interface anlegen 
                x: 0 + Math.random() * 500,
                y: 160,
                dx: 2.5,
        ...
        for (let i: number = 0; i < 5; i++) {           // 5 Skifahrer erzeugen
            skifahrer[i] = {                            // Werte für das Interface festlegen 
                x: 0 + Math.random() * 10,              // Startposition für x 
                y: 300 + Math.random() * 200,           // Startposition für y 
                dx: Math.random() * 5,                 // Geschwindigkeit der Bewegungsrichtung für x
                dy: Math.random() * 3,                  // Geschwindigkeit der Bewegungsrichtung für y 
                colorHut: "hsl(" + Math.random() * 360 + ", 90%, 60%)", //Zufallsfarbe für die Skifahrer erzeugen
    ...
    //Aufgabe 4 
    function drawGondel(_gondel: GondelInfo): void {
        crc2.beginPath();
        crc2.lineWidth = 2.5;
    ...
    //Skifahrer Aufgabe 4
    function drawSki(_skifahrer: SkiInfo): void {  // Parameter für die Funktion drawSki aus dem Interface SkiInfo entnehmen 
        crc2.beginPath();
        crc2.fillStyle = "black";
    ...
    function animate(): void {
        ...
        //Skifahrer Aufgabe 4 
        for (let i: number = 0; i < skifahrer.length; i++) { // i druchläuft das Array des Skifahrers
